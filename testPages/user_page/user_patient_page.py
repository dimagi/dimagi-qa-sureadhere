...
class UserPatientPage(BasePage):
    ...
    def fill_patient_form(self, site, mob='NO'):
        self.wait_for_page_to_load()
        self.wait_for_element('first_name')
        self.wait_for_element('button_SAVE')
        if mob == 'YES':
            self.type('first_name', self.first_name_mob)
            self.type('last_name', self.last_name_mob)
            self.type('mrn', self.mrn_mob)
            self.type('email', self.email_mob)
            self.type('phone_number', UserData.phone_number)
            self.type('user_name', self.username_mob)
        else:
